...
I will not go much in depth of the *node_modules, public, .gitignore, package-lock.json, package.json and the README.md* files/folders since they're generated by React tool ( create-react-app ), NPM and Git.
...
This is the Route with Authentication Component, **You cannot delete it** ( just **edit the condition part** ) since it is crutial for the routes that needs authentication. It is a really straight forward component that makes use of React Router, if the provided condition is true, it will proceed to the desired route, othewise it will be redirected to some provided path. One example usage for this component is any page that needs login or other kind of permission for accessing the provided route.
...

{
    ...
    component: DashboardComponent,   // The component that should be rendered when the URL matches the provided path
...
```javascript
{
    ...
    component: MainComponent,        // The component that should be rendered when the URL matches the provided path
    componentProps: {key: "value"}   // If you want some static values, otherwise just let it as an empty object {}
